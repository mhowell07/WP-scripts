...
1. Copy `wp-batch-search-replace.sh` into the root directory of your WordPress site.
...
1. Enter the string to search for (old URL, path, or value).
2. Enter the replacement string (new URL, path, or value).
3. Choose mode:
   - **1** → Full database (recommended for migrations)
   - **2** → Only large tables (posts, postmeta, options, usermeta)
4. Choose dry-run (`y/n`) to preview changes before committing.
